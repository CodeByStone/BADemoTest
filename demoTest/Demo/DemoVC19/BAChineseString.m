...
@implementation BAChineseString

#pragma mark - 返回排序后的tableview右边的索引indexArray
+ (NSMutableArray*)BA_IndexArray:(NSArray*)stringArr
{
    NSMutableArray *tempArray = [self BA_ReturnSortChineseArrar:stringArr];
    NSMutableArray *A_Result=[NSMutableArray array];
    NSString *tempString ;
    
    for (NSString* object in tempArray)
    {
        NSString *pinyin = [((BAChineseString*)object).pinYin substringToIndex:1];
        // 不同
...
+ (NSMutableArray*)BA_LetterSortArray:(NSArray*)stringArr
{
    ...
    for (NSString* object in tempArray) {
         ...
         NSString *pinyin = [((BAChineseString *)object).pinYin substringToIndex:1];
         NSString *string = ((BAChineseString *)object).string;
        //不同
        if(![tempString isEqualToString:pinyin])
        {
            //分组
...
+ (NSString*)BA_RemoveSpecialCharacter: (NSString *)str {
    NSRange urgentRange = [str rangeOfCharacterFromSet: [NSCharacterSet characterSetWithCharactersInString: @",.？、 ~￥#&<>《》()[]{}【】^@/￡¤|§¨「」『』￠￢￣~@#&*（）——+|《》$_€"]];
    if (urgentRange.location != NSNotFound)
    {
        return [self BA_RemoveSpecialCharacter:[str stringByReplacingCharactersInRange:urgentRange withString:@""]];
    }
    return str;
}

/*! 返回排序好的字符拼音 */
+ (NSMutableArray*)BA_ReturnSortChineseArrar:(NSArray*)stringArr
{
    // 获取字符串中文字的拼音首字母并与字符串共同存放
    NSMutableArray *chineseStringsArray=[NSMutableArray array];
    for(int i=0;i<[stringArr count];i++)
    {
        BAChineseString *chineseString=[[BAChineseString alloc]init];
